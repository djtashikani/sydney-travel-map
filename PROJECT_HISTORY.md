...
**理由:**
- 2026-02-06にVultrサーバーでマルウェア感染が発生
- `/tmp/x86_64.kok` というマルウェアがcrontabとrc.localに仕込まれていた
- サーバーを停止・削除したにも関わらず、帯域使用料として100ドル以上の請求が発生
- SSH接続が不安定（短時間に複数接続するとIPがブロックされる）

**対応:**
- 2026-02-06: Xserver VPSに完全移行
- 2026-02-10: Vultrサーバーを削除、アカウントも削除予定

**今後の方針:**
- Vultrは二度と使用しない
- VPSは国内サービス（Xserver VPS等）を使用する
